#!/usr/bin/env python
"""Cost-robust search — re-pick the entry to survive higher-than-assumed costs.

Stage 6 found the entry's returns are fragile to costs (it trades a lot). This
re-runs the disciplined search with the SAME guards as P1, plus two changes that
target cost-robustness *without* hand-picking or peeking:

* **Wider rebalance bands in the grid** — the structural lever for turnover.
* **Rank under a conservative 2× cost assumption.** We select the config that's
  best assuming real on-chain costs are double our estimate. Higher-turnover
  configs are naturally penalized, so cost-robust ones rise — chosen by the
  objective, not by eyeballing full-data results.

All other discipline is unchanged: rank on TRAIN only, one config across all
tokens, hard drawdown gate vs buy-and-hold, validate the winner ONCE on the
untouched holdout (reported at realistic 1× costs, with 1×/2×/3× robustness).
...
TRAIN_RANK_COST_MULT = 2.0  # rank assuming costs are 2x our estimate

GRID = {
    "target_vol": [0.015, 0.02, 0.025, 0.03, 0.04],
    "trend_period": [50, 100, 150, 200],
    "vol_lookback": [15, 20, 30],
    "rebalance_band": [0.0, 0.03, 0.06, 0.10, 0.15],
}


def _scaled_costs(k: float) -> "config.CostModel":
    b = config.DEFAULT_COSTS
    return config.CostModel(
        swap_fee=b.swap_fee * k, slippage_bps=b.slippage_bps * k, gas_usd=b.gas_usd * k
    )
...
def _evaluate(candles, symbol, cfg, costs=config.DEFAULT_COSTS):
    """Risk-on metrics for one config on one candle segment at given costs."""
    res = run_backtest(
        candles,
        _make(cfg),
        symbol=symbol,
...
def main() -> int:
    ...
    stress_costs = _scaled_costs(TRAIN_RANK_COST_MULT)
    print(
        f"Searching {len(combos)} configs on TRAIN only, ranked at "
        f"{TRAIN_RANK_COST_MULT}x costs ..."
    )
    ...
    for cfg in combos:
        ...
        for symbol in symbols:
            m = _evaluate(data[symbol]["train"], symbol, cfg, costs=stress_costs)
            per_token[symbol] = m
...
def _write_summary(pool, winner, holdout_rows, n_eligible, n_total) -> None:
    ...
    lines.append("")
    path = os.path.join(OUT_DIR, "search_cost_robust_summary.md")
